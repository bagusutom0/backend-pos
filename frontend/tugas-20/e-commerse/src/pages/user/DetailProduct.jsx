...
export default function DetailProduct(props) {
  ...
  function showRatings() {
    let result = '';
    for (let i = 0; i < Number(product.review[0]); i++) {
      result += '★';
    }
  ...
  useEffect(() => {
    ...
    if (product) {
      setDetailImage(product.image[0]);
    }
  }, [product]);
  ...
  return (
    <>
      ...
      <section className="flex justify-between flex-col items-center md:flex-row px-6 py-4">
        ...
        <div className="flex flex-col md:w-1/2 flex-nowrap">
          ...
          <div className="grid grid-cols-4 gap-4 mt-4 h-1/4">
            {product.image.map((tumbnailImage) => (
              <img
                key={`tumbnailImage ${product.image.indexOf(tumbnailImage)}`}
                src={tumbnailImage}
                alt="ring"
        ...
        <div className="flex flex-col justify-between md:w-1/2 md:px-6 pb-10 mt-10 md:mt-0">
          ...
          <h1 className="lg:text-4xl md:text-xl">{product.name}</h1>

          <span className="lg:text-xl md:text-xs">
            {showRatings()} {product.review[1]} Review
          </span>

          <p className="lg:text-2xl mt-2 md:text-lg">
            $ {product.price} <span className="text-sm">tax included.</span>
          </p>
            ...
            <div className="flex justify-between w-16 mt-1">
              <label className="flex items-center">
                ...
                <span
                  ...
                  style={{ backgroundColor: product.colour[0] }}
                ></span>
              </label>
              <label className="flex items-center">
                <input type="radio" name="option" className="hidden peer" />
                <span
                  className="w-4 h-4  rounded-sm hover:border hover:border-black transition ease-in-out delay-75"
                  style={{ backgroundColor: product.colour[1] }}
                ></span>
              </label>
              <label className="flex items-center">
                <input type="radio" name="option" className="hidden peer" />
                <span
                  className="w-4 h-4  rounded-sm hover:border hover:border-black transition ease-in-out delay-75"
                  style={{ backgroundColor: product.colour[2] }}
                ></span>
              </label>
            </div>
          </div>
          ...
          <div className="lg:mt-8 mt-4 grid grid-cols-2 gap-2">
            <button
              className="border border-black rounded-md lg:text-xl md:text-sm lg:p-2 md:p-1 hover:bg-gray-100"
      ...
      <ListProduct
        ...
        subtitle="Make every day special with alluring, high-quality products."
        showAction2
      />

      <section className="mt-10 px-12 flex flex-col md:flex-row justify-between gap-4">
        <div className="md:w-1/2 relative">
          <img src={jewelry} alt="product" className="w-full h-[500px]" />
